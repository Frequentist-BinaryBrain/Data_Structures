class BinaryTree:
    ...
    def inOrderTraversal(self, index):
        # This method performs in-order traversal of the binary tree, starting from the given index.

        # Check if the given index is beyond the last used index, indicating an empty or invalid node.
        if index > self.lastUsedIndex:
            return

        # Recursively perform in-order traversal on the left child (index*2) of the current node.
        self.inOrderTraversal(index * 2)

        # Print the value of the current node during the traversal.
        print(self.customList[index])

        # Recursively perform in-order traversal on the right child (index*2 + 1) of the current node.
